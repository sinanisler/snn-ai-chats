...
class SNN_AI_Chat {
    ...
    public function __construct() {
        ...
        // RAG functionality AJAX actions
    ...
    public function create_database_tables() {
        ...
        // RAG Documents table
        $table_name_documents = $wpdb->prefix . 'snn_rag_documents';
        ...
        // RAG Document Chunks table with vector embeddings
        $table_name_chunks = $wpdb->prefix . 'snn_rag_document_chunks';
        $sql_chunks = "CREATE TABLE $table_name_chunks (
            id mediumint(9) NOT NULL AUTO_INCREMENT,
            ...
            FOREIGN KEY (document_id) REFERENCES {$wpdb->prefix}snn_rag_documents(id) ON DELETE CASCADE
        ) $charset_collate;";
        ...
        // Add progress_log column to existing installations
        $table_name_documents = $wpdb->prefix . 'snn_rag_documents';
        $column_exists = $wpdb->get_results("SHOW COLUMNS FROM $table_name_documents LIKE 'progress_log'");
        if (empty($column_exists)) {
            $wpdb->query("ALTER TABLE $table_name_documents ADD COLUMN progress_log text DEFAULT '' AFTER status");
        }
    }
    ...
    public function admin_menu() {
        ...
        add_submenu_page(
            ...
            'RAG Documents',
            'manage_options',
            'snn-ai-chat-rag-documents',
            array($this, 'rag_documents_page')
        );
    ...
    public function settings_page() {
        ...
        <div class="wrap">
            ...
            <form method="post" action="" class="snn-settings-form" id="snn-settings-form">
                ...
                <div class="bg-white p-6 rounded-lg shadow mb-4" id="snn-rag-settings-section">
                    <h2 class="text-xl font-semibold mb-4">RAG (Retrieval-Augmented Generation) Settings</h2>
                    
                    <div class="mb-4">
                        <label class="flex items-center">
                            <input type="checkbox" name="rag_enabled" value="1" <?php checked($settings['rag_enabled'], 1); ?> class="mr-2">
                            <span class="text-sm font-medium text-gray-700 snn-tooltip" data-tippy-content="Enable RAG to allow AI to use uploaded documents as knowledge base">
                                Enable RAG (Retrieval-Augmented Generation)
                            </span>
                        </label>
                        <p class="text-sm text-gray-600 mt-1">When enabled, the AI can reference uploaded documents to provide more accurate and contextual responses.</p>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="mb-4">
                            <label for="rag_chunk_size" class="block text-sm font-medium text-gray-700 mb-2 snn-tooltip" data-tippy-content="Size of text chunks when splitting documents (characters)">
                                Chunk Size
                            </label>
                            <input type="number" min="100" max="4000" id="rag_chunk_size" name="rag_chunk_size" value="<?php echo esc_attr($settings['rag_chunk_size']); ?>" class="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                        </div>
                        <div class="mb-4">
                            <label for="rag_chunk_overlap" class="block text-sm font-medium text-gray-700 mb-2 snn-tooltip" data-tippy-content="Overlap between chunks to maintain context (characters)">
                                Chunk Overlap
                            </label>
                            <input type="number" min="0" max="500" id="rag_chunk_overlap" name="rag_chunk_overlap" value="<?php echo esc_attr($settings['rag_chunk_overlap']); ?>" class="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                        </div>
                        <div class="mb-4">
                            <label for="rag_max_results" class="block text-sm font-medium text-gray-700 mb-2 snn-tooltip" data-tippy-content="Maximum number of relevant chunks to include in context">
                                Max Results
                            </label>
                            <input type="number" min="1" max="10" id="rag_max_results" name="rag_max_results" value="<?php echo esc_attr($settings['rag_max_results']); ?>" class="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                        </div>
                        <div class="mb-4">
                            <label for="rag_similarity_threshold" class="block text-sm font-medium text-gray-700 mb-2 snn-tooltip" data-tippy-content="Minimum similarity score for including chunks (0.0-1.0)">
                                Similarity Threshold
                            </label>
                            <input type="number" step="0.01" min="0.0" max="1.0" id="rag_similarity_threshold" name="rag_similarity_threshold" value="<?php echo esc_attr($settings['rag_similarity_threshold']); ?>" class="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                        </div>
                    </div>

                    <div class="mb-4">
                        <label for="embedding_api_provider" class="block text-sm font-medium text-gray-700 mb-2 snn-tooltip" data-tippy-content="API provider for generating embeddings">
                            Embedding API Provider
                        </label>
                        <select id="embedding_api_provider" name="embedding_api_provider" class="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                            <option value="openai" <?php selected($settings['embedding_api_provider'], 'openai'); ?>>OpenAI</option>
                        </select>
                    </div>

                    <div class="mb-4">
                        <label for="embedding_model" class="block text-sm font-medium text-gray-700 mb-2 snn-tooltip" data-tippy-content="Model used for generating text embeddings">
                            Embedding Model
                        </label>
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-md p-4">
                        <h4 class="text-sm font-medium text-blue-800 mb-2">RAG Requirements:</h4>
                        <ul class="text-sm text-blue-700 space-y-1">
                            <li>• OpenAI API key is required for generating embeddings</li>
                            <li>• Supported document formats: TXT, PDF, DOC, DOCX</li>
                            <li>• Documents are chunked and stored as vector embeddings</li>
                            <li>• Relevant chunks are automatically included in chat context</li>
                        </ul>
                    </div>
                </div>
    ...
    private function get_default_chat_settings() {
        return array(
            ...
            'frequency_penalty' => 0.0,
            'presence_penalty' => 0.0,
            'use_rag' => 0, // Individual chat can enable/disable RAG
        );
    }
    ...
    public function rag_documents_page() {
        $settings = $this->get_settings();
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('RAG Documents', 'snn-ai-chat'); ?></h1>
            
            <?php if (empty($settings['rag_enabled'])): ?>
                <div class="notice notice-warning">
                    <p><?php esc_html_e('RAG is currently disabled. Enable it in Settings to upload documents.', 'snn-ai-chat'); ?></p>
                </div>
            <?php endif; ?>
            ...
            <details class="snn-rag-upload-section" style="background: #fff; padding: 20px; margin: 20px 0; border: 1px solid #ccd0d4; border-radius: 4px;">
                ...
                <form id="snn-rag-upload-form" enctype="multipart/form-data">
                    ...
                    <table class="form-table">
                        <tr>
                            <th scope="row"><?php esc_html_e('Select File', 'snn-ai-chat'); ?></th>
                            <td>
                    ...
                    <p class="submit">
                        <button type="submit" class="button button-primary" <?php echo empty($settings['rag_enabled']) ? 'disabled' : ''; ?>>
                            <?php esc_html_e('Upload Document', 'snn-ai-chat'); ?>
                        </button>
                    </p>
                </form>
                    ...
                    <table class="wp-list-table widefat striped">
                        <thead>
                            <tr>
                                ...
                                <th><?php esc_html_e('Type', 'snn-ai-chat'); ?></th>
                                <th><?php esc_html_e('Size', 'snn-ai-chat'); ?></th>
                                <th><?php esc_html_e('Chunks', 'snn-ai-chat'); ?></th>
                                <th><?php esc_html_e('Status', 'snn-ai-chat'); ?></th>
                                <th><?php esc_html_e('Upload Date', 'snn-ai-chat'); ?></th>
                                <th><?php esc_html_e('Actions', 'snn-ai-chat'); ?></th>
                            </tr>
                        </thead>
        ...
        jQuery(document).ready(function($) {
            ...
            // Handle file upload
            $('#snn-rag-upload-form').on('submit', function(e) {
                e.preventDefault();
                var fileInput = $('#rag_document')[0];
                if (!fileInput.files.length) {
                    alert('Please select a file to upload.');
                    return;
                }
                var formData = new FormData();
                formData.append('action', 'snn_upload_document');
                formData.append('nonce', snn_ai_chat_ajax.nonce);
                formData.append('rag_document', fileInput.files[0]);
                $('#upload-progress').show();
                $('#progress-bar').css('width', '0%');
                $('#upload-status').text('Uploading...');
                $('#processing-console').show().text('');
                // Add initial log
                addConsoleLog('Uploading file...');
                $.ajax({
                    url: snn_ai_chat_ajax.ajax_url,
                    type: 'POST',
                    data: formData,
                    ...
                    xhr: function() {
                        ...
                        xhr.upload.addEventListener("progress", function(evt) {
                            if (evt.lengthComputable) {
                                ...
                                addConsoleLog('Upload progress: ' + percentComplete.toFixed(1) + '%');
                            }
                        }, false);
                        return xhr;
                    },
                    success: function(response) {
                        if (response.success) {
                            $('#upload-status').text('File uploaded! Starting chunk processing...');
                            $('#progress-bar').css('width', '100%');
                            addConsoleLog('File uploaded. Starting chunk processing...');
                            // Start monitoring processing progress
                            monitorProcessingProgress(response.data.document_id);
                        } else {
                            addConsoleLog('Error: ' + (response.data || 'Upload failed'));
                            alert('Error: ' + (response.data || 'Upload failed'));
                            $('#upload-progress').hide();
                        }
                    },
                    error: function() {
                        addConsoleLog('Upload failed. Please try again.');
            ...
            function addConsoleLog(msg) {
                var console = $('#processing-console');
                var now = new Date();
            ...
            // Function to monitor processing progress
            function monitorProcessingProgress(documentId) {
                var console = $('#processing-console');
                var lastStatus = '';
                var lastChunks = 0;
                addConsoleLog('Extracting text from document...');
                var pollInterval = setInterval(function() {
                    $.ajax({
                        url: snn_ai_chat_ajax.ajax_url,
                        type: 'POST',
                        data: {
                            action: 'snn_check_processing_status',
                            nonce: snn_ai_chat_ajax.nonce,
                            ...
                            if (response.success) {
                                ...
                                if (data.status !== lastStatus) {
                                    if (data.status === 'processing') {
                                        addConsoleLog('Splitting text into chunks and generating embeddings...');
                                    }
                                    if (data.status === 'completed') {
                                        addConsoleLog('✓ Processing completed! ' + data.total_chunks + ' chunks created.');
                                    }
                                    if (data.status === 'failed') {
                                        addConsoleLog('✗ Processing failed!');
                                    }
                                    lastStatus = data.status;
                                }
                                // Log chunk count changes
                                if (data.total_chunks > lastChunks && data.status === 'completed') {
                                    addConsoleLog('Total chunks: ' + data.total_chunks);
                                    lastChunks = data.total_chunks;
                                }
                                if (data.status === 'completed') {
                                    clearInterval(pollInterval);
                                    $('#upload-status').html('<span style="color: #46b450; font-weight: bold;">✓ Processing completed successfully!</span>');
                                    setTimeout(function() {
                                        $('#upload-progress').hide();
                                        // Do NOT hide the console log
                                        $('#snn-rag-upload-form')[0].reset();
                                        loadDocumentsList();
                                        alert('Document uploaded and processed successfully! ' + data.total_chunks + ' chunks created.');
                                    }, 3000);
                                } else if (data.status === 'failed') {
                                    clearInterval(pollInterval);
                                    $('#upload-status').html('<span style="color: #dc3232; font-weight: bold;">✗ Processing failed!</span>');
                                    setTimeout(function() {
                                        $('#upload-progress').hide();
                                        // Do NOT hide the console log
                                        loadDocumentsList();
                        ...
                        error: function() {
                            clearInterval(pollInterval);
                            addConsoleLog('✗ Error checking progress!');
                            $('#upload-status').html('<span style="color: #dc3232; font-weight: bold;">✗ Error checking progress!</span>');
                            setTimeout(function() {
                                $('#upload-progress').hide();
                                // Do NOT hide the console log
                            }, 3000);
            ...
            function loadDocumentsList() {
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        action: 'snn_get_documents',
                        nonce: snn_ai_chat_ajax.nonce
                    },
                    success: function(response) {
                        if (response.success) {
                            var tbody = $('#documents-table-body');
                            tbody.empty();
                            
                            if (response.data.length === 0) {
                                tbody.append('<tr><td colspan="7">No documents uploaded yet.</td></tr>');
                            } else {
                                response.data.forEach(function(doc) {
                                    var statusClass = doc.status === 'completed' ? 'success' : 
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            alert('Error deleting document: ' + (response.data || 'Unknown error'));
                        }
                    }
                });
            });
        ...
        .snn-rag-upload-section { margin: 20px 0; }
        ...
        #processing-console::-webkit-scrollbar {
            width: 8px;
        }
        #processing-console::-webkit-scrollbar-track {
            background: #2e2e2e;
        }
        #processing-console::-webkit-scrollbar-thumb {
            background: #555;
            border-radius: 4px;
        }
        #processing-console::-webkit-scrollbar-thumb:hover {
            background: #777;
        }
        </style>
        <?php
    ...
    public function handle_document_upload() {
        ...
        $settings = $this->get_settings();
        if (empty($settings['rag_enabled'])) {
            wp_send_json_error('RAG is not enabled.');
        }
        ...
        $allowed_types = array('txt', 'pdf', 'doc', 'docx');
        $file_extension = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
        
        if (!in_array($file_extension, $allowed_types)) {
            wp_send_json_error('File type not supported.');
        }
        ...
        $rag_dir = $upload_dir['basedir'] . '/snn-ai-chat-rag/';
        if (!file_exists($rag_dir)) {
            wp_mkdir_p($rag_dir);
        }
        ...
        global $wpdb;
        $table_name = $wpdb->prefix . 'snn_rag_documents';
        ...
        if ($result === false) {
            unlink($filepath);
            wp_send_json_error('Failed to save document record.');
        }

        $document_id = $wpdb->insert_id;

        // Process document in background
        $this->process_document_for_rag($document_id, $filepath, $file_extension);

        wp_send_json_success(array('document_id' => $document_id));
    }

    // Process document and create embeddings
    private function process_document_for_rag($document_id, $filepath, $file_type) {
        global $wpdb;
        
        try {
            // Extract text from document
            $text_content = $this->extract_text_from_file($filepath, $file_type);
            if (empty($text_content)) {
                throw new Exception('Could not extract text from document.');
            }
            // Split text into chunks
            $settings = $this->get_settings();
            $chunks = $this->split_text_into_chunks($text_content, $settings['rag_chunk_size'], $settings['rag_chunk_overlap']);
            $total_chunks = count($chunks);
            $chunks_table = $wpdb->prefix . 'snn_rag_document_chunks';
            $successful_chunks = 0;
            foreach ($chunks as $index => $chunk) {
                // Generate embedding for chunk
                $embedding = $this->generate_embedding($chunk, $settings);
                if ($embedding) {
                    $result = $wpdb->insert(
                        $chunks_table,
                        array(
            ...
            $documents_table = $wpdb->prefix . 'snn_rag_documents';
            $wpdb->update(
                $documents_table,
                array(
                    'status' => 'completed',
            ...
            error_log('RAG document processing failed: ' . $e->getMessage());
        }
    }

    // Extract text from various file types
    private function extract_text_from_file($filepath, $file_type) {
        switch ($file_type) {
            case 'txt':
                return file_get_contents($filepath);
                
            case 'pdf':
                // For PDF, you might want to use a library like smalot/pdfparser
                // For now, we'll return a basic implementation
                return $this->extract_text_from_pdf($filepath);
                
            case 'doc':
            case 'docx':
                // For DOC/DOCX, you might want to use PhpOffice/PhpWord
                return $this->extract_text_from_word($filepath);
    ...
    private function extract_text_from_pdf($filepath) {
        ...
        // For production, consider using libraries like smalot/pdfparser
        $text = '';
        if (function_exists('shell_exec') && !empty(shell_exec('which pdftotext'))) {
            $text = shell_exec('pdftotext "' . $filepath . '" -');
        }
        return $text ?: 'PDF text extraction not available. Please upload a TXT file instead.';
    }

    // Basic Word document text extraction
    private function extract_text_from_word($filepath) {
        // This is a placeholder - for production use PhpOffice/PhpWord
        return 'Word document text extraction not implemented. Please upload a TXT file instead.';
    }
    ...
    private function split_text_into_chunks($text, $chunk_size = 1000, $overlap = 200) {
        ...
        foreach ($sentences as $sentence) {
            ...
            if ($current_size + $sentence_length > $chunk_size && !empty($current_chunk)) {
                ...
                $overlap_words = array_slice($words, max(0, $word_count - ($overlap / 5)));
    ...
    private function generate_embedding($text, $settings) {
        $api_key = $settings['openai_api_key'];
        $model = $settings['embedding_model'];
        
        if (empty($api_key)) {
            error_log('OpenAI API key not configured for embeddings');
            return false;
        }
        ...
        error_log('Embedding generation failed: ' . $response);
        return false;
    }

    // Get relevant context from RAG documents
    private function get_rag_context($query, $settings) {
        global $wpdb;
        
        // Generate embedding for the query
        $query_embedding = $this->generate_embedding($query, $settings);
        if (!$query_embedding) {
            return '';
        }
        ...
        $chunks_table = $wpdb->prefix . 'snn_rag_document_chunks';
        $chunks = $wpdb->get_results("SELECT content, embedding_vector FROM $chunks_table");

        $similarities = array();
        
        foreach ($chunks as $chunk) {
            $chunk_embedding = json_decode($chunk->embedding_vector, true);
            if ($chunk_embedding) {
        ...
        $top_chunks = array_slice($similarities, 0, $settings['rag_max_results']);
        $context_parts = array_map(function($chunk) {
    ...
    // Get documents list for admin page
    public function get_documents_list() {
        ...
        $documents = $wpdb->get_results("SELECT * FROM $table_name ORDER BY upload_date DESC");
    ...
    public function handle_document_delete() {
        ...
        $documents_table = $wpdb->prefix . 'snn_rag_documents';
        $document = $wpdb->get_row($wpdb->prepare("SELECT filename FROM $documents_table WHERE id = %d", $document_id));
        
        if ($document) {
            // Delete file
            $upload_dir = wp_upload_dir();
            $filepath = $upload_dir['basedir'] . '/snn-ai-chat-rag/' . $document->filename;
            if (file_exists($filepath)) {
                unlink($filepath);
            }
            
            // Delete database records (chunks will be deleted by foreign key constraint)
    ...
    public function check_processing_status() {
        ...
        $document = $wpdb->get_row($wpdb->prepare(
            "SELECT status, total_chunks FROM $table_name WHERE id = %d", 
            $document_id
        ));
        if (!$document) {
            wp_send_json_error('Document not found.');
